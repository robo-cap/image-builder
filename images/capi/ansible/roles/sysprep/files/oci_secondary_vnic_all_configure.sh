...
oci_vcn_ip_routing_add() {
    ...
    if [ -n "$ns" ]; then
        ...
        ip route replace default via $virtrt dev $iface || oci_vcn_err "cannot replace default route via $virtrt on interface $iface"
        oci_vcn_info "replaced default route via $virtrt on interface $iface with index $nic_i"
    fi
}
